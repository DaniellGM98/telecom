<?php
	...
	$app->group('/prod_entrada_detalle/', function() {
		...
		$this->get('getListaSkuDisp2/{producto_id}[/{sucursal_id}[/{sku}]]', function($request, $response, $arguments) {
			$arguments['sucursal_id'] = isset($arguments['sucursal_id'])? $arguments['sucursal_id']: null;
			$arguments['sku'] = isset($arguments['sku'])? $arguments['sku']: null;
			$venta = $this->model->prod_entrada_detalle->getSkuVenta($arguments['producto_id'])->result;
			$sku_venta = array();
			foreach($venta as $item){
				$sku_venta[] = $item->sku;
			}			
			$apartado_traspaso = $this->model->prod_entrada_detalle->getSkuApartadoTraspaso($arguments['producto_id']);
			foreach($apartado_traspaso as $item2){
				$nomSuc = $this->model->prod_entrada_detalle->getNameSucursal($item2->sucursal)->nombre;
				$item2->sucursal = $nomSuc;
			}			
			$disponibles = $this->model->prod_entrada_detalle->getListaSkuDisp2($arguments['producto_id'], $arguments['sucursal_id'], $arguments['sku'], $sku_venta);
			$disponibles->skuDisponibles = [];
			$final = array();
			$count = 0;
			foreach($disponibles->result as $disponible) {
				foreach($apartado_traspaso as $item2){
					if($disponible->sku == $item2->sku && $disponible->sucursal == $item2->sucursal){
							unset($disponibles->result[$count]);
					}
				}
				$count++;
				$disponible->fechaEntrada = $this->model->prod_entrada_detalle->getFechaEntradaSkuDisp($disponible->sku)->fecha;
